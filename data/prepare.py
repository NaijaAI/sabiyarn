...
def run():
    ...
    def process(example):
        ...
        ids = enc.encode(example['text_yor']) # encode_ordinary ignores any special tokens   encode_ordinary
    ...
    if isinstance(files, str):
        ...
        for file in files:
            ...
            dataset = load_dataset(config["dataset"], data_files=[file],
                                   num_proc=num_proc_load_dataset,
                                   verification_mode='no_checks')
            ...
            for split, dset in tokenized.items():
                ...
                if os.path.exists(filename):
                    arr = np.memmap(
                        filename, dtype=dtype, mode='r+', shape=(arr_len,))
                else:
                    arr = np.memmap(
