...
data = load_dataset("damilojohn/lang_id_eng", token = "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")
LOG.info("loaded dataset .....")

model = AutoModelForCausalLM.from_pretrained("BeardedMonster/sabiyarn-125m", trust_remote_code=True)
...
def preprocess_data(row):
    ...
    tokens = tokenizer(row['texts'], return_tensors="pt", truncation=True, padding="max_length", max_length=512, add_special_tokens=True)
    input_ids = tokens.input_ids
    pad = torch.full((len(input_ids), 1), -100, dtype=input_ids.dtype, device=input_ids.device)
...
tokenized_data = data.map(preprocess_data, batched=True, batch_size=64,).with_format("torch")
...
def collate_fn(batch):
    ...
    inputs = torch.stack([item['idx'].clone().detach().to(dtype=torch.int64) for item in batch])
    labels = torch.stack([item['targets'].clone().detach().to(dtype=torch.int64) for item in batch])
    attn_mask = torch.stack([item["attn_mask"].clone().detach().to(dtype=torch.int64) for item in batch])
...
num_train_steps = len(train_dataloader) * 3 * gradient_accumulation_steps  # num_train_epochs = 3
            ...
            if (step + 1) % eval_steps == 0 or (step == len(train_dataloader) - 1):
                ...
                with torch.no_grad():
                    for val_batch in val_dataloader:
                        ...
                        val_attn_mask = create_causal_mask(val_batch['attn_mask'].to(device), device=device)

                        val_outputs = model(idx=val_inputs, targets=val_labels, attn_mask=val_attn_mask)
                ...
                LOG.info(f"Epoch {epoch + 1}, Step {step + 1}/{len(train_dataloader)}: Train Loss = {avg_train_loss}, Val Loss = {avg_val_loss}")
                ...
                if (step + 1) % save_steps == 0 and val_loss <= best_val:
                        ...
                        model.push_to_hub(new_repo_name, commit_message=f"Saving model at step {step + 1}", token=write_token)
    model.push_to_hub(new_repo_name, commit_message = f"final model save", token=write_token)
