...
class MultiTokenLoss(nn.Module):
    ...
    def forward(
        self,
        multi_token_logits: torch.Tensor,
        targets: torch.Tensor,
        mtp_hidden_states: Optional[torch.Tensor] = None,
        mtp_output_heads: Optional[nn.ModuleList] = None,
    ) -> torch.Tensor:
        """
        ...
        for i in range(num_pred_tokens):
            ...
            if target_end <= targets.shape[1]:
                ...
                if (
                    self.use_cut_cross_entropy
                    and mtp_hidden_states is not None
                    and mtp_output_heads is not None
