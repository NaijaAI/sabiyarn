...
class DeepseekV2RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states - hidden_states * torch.rsqrt(variance + self.variance_epsilon)
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    b, h, s, d = q.shape # [batch_size, num_attention_heads, seq_len, head_size]
    q = q.view(b ,h ,s, d // 2, 2).transpose(4,3).reshape(b, h, s, d)
    b, h, s, d = k.shape # [batch_size, num_attention_heads, seq_len, head_size]
...
class MLA(nn.Module):
    ...
    def forward(self, hidden_states, position_ids, attention_mask=None):
        ...
        output = attn_weights @ value_states # (b, num_head, q_len, q_len)
