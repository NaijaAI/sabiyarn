...
class MemoryLayer(nn.Module):
    def __init__(self, hidden_size, memory_size=100, memory_dim=768):
        super().__init__()
        self.memory = nn.Parameter(torch.randn(memory_size, memory_dim))  # Learnable memory matrix
...
class ShortTermMemoryLayer(nn.Module):
    ...
    def __init__(self, hidden_size, memory_size=10, memory_dim=768):
        super().__init__()
        self.memory = nn.Parameter(torch.randn(memory_size, memory_dim))  # Temporary memory initialized per batch
        self.query_transform = nn.Linear(hidden_size, memory_dim)
        self.key_transform = nn.Linear(memory_dim, memory_dim)
...
class PersistentMemoryLayer(nn.Module):
    ...
    def __init__(self, hidden_size, memory_size=100, memory_dim=768):
        ...
        self.memory = nn.Parameter(torch.randn(memory_size, memory_dim), requires_grad=False)  # Frozen by default
        self.query_transform = nn.Linear(hidden_size, memory_dim)
        self.key_transform = nn.Linear(memory_dim, memory_dim)
    ...
    def update_memory(self, x, threshold=0.7):
        ...
        with torch.no_grad():
            ...
            if torch.max(similarity_scores) < threshold:  # Update if no close match found
...
class PersistentMemoryWithUsage(nn.Module):
    ...
    def __init__(self, hidden_size, memory_size=100, memory_dim=768):
        ...
        self.usage_counts = torch.zeros(memory_size, dtype=torch.int)  # Track memory usage
        self.query_transform = nn.Linear(hidden_size, memory_dim)
        self.memory_update_layer = nn.Linear(hidden_size, memory_dim)
