...
from sabiyarn.differential_attention import (
    DiffAttention,
    DiffAttnArgs,
...
def test_diff_attention():
    ...
    try:
        ...
        output_explicit_mask = model(
            x, start_pos=0, freqs_cis=freqs_cis, mask=causal_mask
        )
        ...
        assert (
            output_no_mask.shape == expected_shape
        ), f"Expected {expected_shape}, got {output_no_mask.shape}"
