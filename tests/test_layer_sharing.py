...
def test_layer_sharing():
    ...
    try:
        ...
        traditional_params = sum(
            p.numel() for p in traditional_model.parameters() if p.requires_grad
        )
        ...
        layer_sharing_params = sum(
            p.numel() for p in layer_sharing_model.parameters() if p.requires_grad
        )
        ...
        print(
            f"   Memory reduction: {memory_reduction:,} parameters ({memory_reduction_pct:.1f}%)"
        )
        ...
        print(
            f"✅ Architecture correct: {layer_sharing_model.n_unique_layers} unique layers, {layer_sharing_model.repeat_factor}x repeat factor"
        )
...
def test_layer_sharing_validation():
    ...
    try:
        ...
        try:
            ModelArgs(
                **base_config, layer_sharing=True, n_unique_layers=5
            )  # 12 % 5 != 0
        ...
        config = ModelArgs(
            **base_config, layer_sharing=True, n_unique_layers=4
        )  # 12 % 4 == 0
        print(
            f"✅ Valid configuration accepted: {config.n_layers} layers, {config.n_unique_layers} unique"
        )
