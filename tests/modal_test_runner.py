...
def run_tests_on_gpu():
    ...
    def test_cut_cross_entropy():
        ...
        try:
            ...
            if device == "cpu":
                ...
                e = torch.randn(
                    batch_size, seq_len, embed_dim, device=device
                )  # embeddings
                c = torch.randn(
                    vocab_size, embed_dim, device=device
                )  # classifier weights
    ...
    def test_mha_model_initialization():
        ...
        try:
            ...
            ), f"Hidden states shape mismatch: {hidden_states.shape} vs {expected_hidden}"
            assert (
                logits.shape == expected_logits
            ), f"Logits shape mismatch: {logits.shape} vs {expected_logits}"
    ...
    def test_differential_attention_model():
        ...
        try:
            ...
            assert (
                hidden_states.shape == expected_hidden
            ), f"Hidden states shape mismatch"
    ...
    def test_gqa_model():
        ...
        try:
            gqa_config = GQAArgs(
                dim=384, n_kv_heads=4, n_heads=8, max_seq_len=32, max_batch_size=2
            )
            config = ModelArgs(
                dim=384,
                n_layers=2,
    ...
    def test_mla_with_moe():
        ...
        try:
            ...
            assert (
                hidden_states.shape == expected_hidden
            ), f"Hidden states shape mismatch: {hidden_states.shape} vs {expected_logits}"
            assert (
                logits.shape == expected_logits
            ), f"Logits shape mismatch: {logits.shape} vs {expected_logits}"
            ...
            ), "Layer should have attention component"
            assert hasattr(
                first_layer, "feed_forward"
            ), "Layer should have MoE feed_forward component"

            from sabiyarn.moe import MoE

            assert isinstance(
                first_layer.feed_forward, MoE
            ), "Should be using MoE for feed_forward"
