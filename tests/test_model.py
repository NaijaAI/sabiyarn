...
def test_mla_model():
    ...
    try:
        ...
        input_ids = torch.Tensor(
            [[23, 68, 126, 18, 555, 68, 72, 39, 45, 67, 91, 804]]
        ).to(torch.int64)

        model.eval()
        with torch.no_grad():
            generated = model.generate(
                input_ids, max_new_tokens=50, use_multi_token=False
            )
...
def load_sabiyarn_checkpoint(
    checkpoint_path="/mnt/c/Users/Jeffrey Paul/Downloads/ckpt_0000360.pt",
    device="cpu",
    max_tokens=50,
    ...
    with torch.no_grad():
        generated = model.generate(
            input_ids, max_new_tokens=max_tokens, use_multi_token=model.use_multi_token
        )
...
def load_test_sabiyarn_checkpoint(
    ...
    for lang, data_list in dummy_data.items():
        ...
        for each in data_list:
            ...
            input_ids = (
                input_ids[:, 1:].to(torch.int64)
                if input_ids[0][0] == 128000
                else input_ids
            )
            ...
            output_text = tokenizer.decode(
                generated[0].tolist()[input_len:], skip_special_tokens=False
            )
