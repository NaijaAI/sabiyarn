...
attention_type = ModelArgs.attention_type
n_heads = DiffAttnArgs.n_heads
n_layers = ModelArgs.n_layers
embed_dim = ModelArgs.dim
...
def _prepare_mask_(b=None, block_size = 1024, eval=False):
    if eval:
        return None
...
def generate_and_decode_sequences(batch_token_ids, model, tokenizer, max_new_tokens=50):
    import torch
...
def train():
    ...
    while True:
        ...
        if iter_num % 512 == 0 and master_process == 0:
