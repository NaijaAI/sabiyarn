...
def configure_optimizer(model, optimizer_type='adam', learning_rate=1e-3, weight_decay=0.01, betas=(0.9, 0.999)):
    if optimizer_type == 'sgd':
        ...
        optimizer = optim.Adam8bit(model.parameters(), lr=learning_rate, betas=betas, weight_decay=weight_decay)
    
    else:
        raise ValueError(f"Optimizer type '{optimizer_type}' not recognized. Please choose from 'sgd', 'adam', 'adamw', 'adam8bit'.")
...
if init_from == 'scratch':
    ...
    if use_differential_attention:
        diff_attn_args = DiffAttnArgs(max_batch_size=max_batch_size, n_heads = n_heads, embed_dim= dim, n_heads =n_heads,
                                      n_kv_heads=n_kv_heads, max_seq_len=block_size, norm_eps=norm_eps)
    else:
        diff_attn_args = None
        
    model_args = ModelArgs(dim, n_layers, n_heads, n_kv_heads, vocab_size, multiple_of, ffn_dim_multiplier, norm_eps, moe, logic_network,
                           max_batch_size, max_seq_len, use_j, diff_attn_args)
...
def _prepare_mask_(b=None, block_size = block_size, eval=False):
    ...
    attn_mask = torch.tril(torch.ones(block_size, block_size)).view(1, 1, block_size, block_size)  
