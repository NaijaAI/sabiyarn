...
def train_sabiyarn(volume: modal.Volume):
    ...
    clear_dedup_hash_folder(
        conf["hash"]["registry_cache"], rehash=conf["data"]["rehash"]
    )
    ...
    if config.dist:
        ...
        train_script = "/app/training/new_train.py"
        ...
        try:
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"torchrun failed with exit code {process.returncode}"
                )
